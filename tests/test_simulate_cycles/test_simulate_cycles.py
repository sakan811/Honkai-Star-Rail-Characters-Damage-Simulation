...
class MockCharacter(Character):
    ...
    def _use_basic_atk(self) -> None:
        ...
        self._update_skill_point_and_ult_energy(skill_points=1, ult_energy=self.BASIC_ATK_ENERGY_GAIN)
    ...
    def _calculate_damage(self, skill_multiplier: float, break_amount: int, 
                         dmg_multipliers: List[float] = None, can_crit: bool = True) -> float:
        ...
        self.current_enemy_toughness = max(0, self.current_enemy_toughness - break_amount)
...
def test_summon_initialization():
    ...
    result = simulate_cycles_for_character_with_summon(topaz, numby, max_cycles=5, simulate_round=1)
    ...
    result = simulate_cycles_for_character_with_summon(jingyuan, lightning_lord, max_cycles=5, simulate_round=1)
...
def test_cycles_action_value_calculation():
    ...
    for max_cycles, expected_action_val in test_cases:
        ...
        assert len(result['DMG']) > 0  # Should have some actions with the given action value
